...
characterisation, and the wider landscape-analysis
literature~\cite{malan2021survey} is likewise diagnostic rather than
generative. Whether an online estimate is accurate enough to drive search is
open, and the controls developed here would settle it.

The practical consequence follows directly. Betweenness costs
$O(N\!\cdot\!M)$ and roughly $30\%$ of per-iteration runtime, and may be
replaced by any heterogeneous weighting at no measurable loss, or by uniform
weighting at about $0.17$ rank units: compute the cheapest centrality
available, or none.
...
predictive one, and the distinction matters: a predictor must be right about
the landscape, whereas an adaptive rule need only be right about which of a few
alternatives is currently doing better. The weaker requirement is the
achievable one here---the predictive route fails at $D=10$ and on the penalised
functions, while the adaptive route beats its own randomisation on $24$ of $28$
functions.
...
same densities on $24$ of $28$ functions, so the benefit is not merely from
varying density, which both arms share. Reaching that conclusion required
correcting our own first reading, since under per-function Holm correction
\textsc{Ads} wins on only $3$ of $28$ and reads as weak; the diagnostic is the
direction count, visible only when the suite is the unit of analysis
(Section~\ref{sec:target2:views} gives the mirror case).
