...
Table~\ref{tab:placebo} and Fig.~\ref{fig:forest} give the comparison between
...
\caption{Effect of destroying the structure--weight correspondence, per
function, with bootstrap intervals. Shading marks the negligible band
$|\delta|<0.147$. No interval excludes zero after correction.}
\label{fig:forest}
\end{figure*}
...
Fig.~\ref{fig:equivalence} shows the intervals directly. Most sit inside the
band but cross its edge, which is the visual form of the distinction this
subsection draws: failing to detect a difference is not the same as
establishing its absence, and only the intervals lying wholly inside the margin
do the latter.
...
permutation control. Shading marks the negligible band $|\delta|<0.147$; an
interval contained in it establishes equivalence at $\alpha=0.05$. Most
intervals straddle the boundary, so the per-function question stays open even
where the difference test is comfortably non-significant.}
...
Mean ranks over the six arms at $D=30$ (Fig.~\ref{fig:ranks}) place the two
sparse arms first (\textsc{Social} $3.121$, \permcontrol{} $3.170$), then
uniform mixing ($3.335$), with the density-varying arms last
($3.676$--$3.977$). That \textsc{Social} and \permcontrol{} are adjacent,
separated by $0.05$ rank units with overlapping standard errors, is the finding
of Section~\ref{sec:res:placebo} in another form. Per-function medians for all
six arms are given in Table~\ref{tab:cec30}.
...
matters (Fig.~\ref{fig:density}).

\begin{figure}[t]
\centering
\includegraphics{fig_density_law}
\caption{Preference for dense neighbourhoods against landscape ruggedness,
over the $30$ configurations in which density has a significant effect. The
association is moderate and carried largely by the most rugged functions.}
\label{fig:density}
\end{figure} That correlation is carried largely by the three most rugged
