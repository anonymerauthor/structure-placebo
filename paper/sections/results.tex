...
(Section~\ref{sec:leverage}). We measure the normalised displacement $\rho$ of
\eqref{eq:relshift} for every graph configuration used in the original report
and in its reference implementation; the supplementary material plots them all.
...
mean by roughly a third of the population's spread, which is not a small
correction. The coefficient of variation of betweenness over nodes is
$0.65$--$0.89$, so the weights are strongly heterogeneous rather than
near-uniform.
...
Per function the picture is different. Raising the runs from $30$ to $51$
...
equivalent functions only from $1$ to $6$ of $28$, because equivalence at
margin $m$ requires $|\hat{\delta}|+\text{half-width}<m$ rather than merely
$\text{half-width}<m$. With a median $|\hat{\delta}|$ of $0.063$ the median
function would need roughly $100$ runs, and on $4$ of $28$ the estimate already
exceeds $0.147$, so no sample size places those inside that band: a genuine, if
small, difference is present there. At the small-effect margin $|\delta|<0.33$,
$26$ of $28$ are equivalent. The supplementary material gives the per-function
intervals and the margin sensitivity.
...
times stricter than negligibility. Per function, effects are bounded within a
small margin, a handful sit near the negligibility boundary, and none survives
correction as a difference.
...
one. We therefore ran the same controls on four further mechanisms---two more
within this optimizer, two in the second target of Section~\ref{sec:target2}---%
on the same suite, at the same sample size and with the same statistic.
Fig.~\ref{fig:calibration} places all five side by side. This subsection covers
the two within this optimizer; the other two are discussed with the second
target.
...
\caption{The same controls on five mechanisms across two optimizers, on
