...
nodes yields an operator that equals uniform mixing in expectation while
retaining the original's weight heterogeneity in every realisation
(Proposition~\ref{prop:mean}), which is exactly the separation an ablation
fails to make; a leverage bound (Proposition~\ref{prop:leverage}) states when
such a comparison is informative at all and identifies the topologies on which
the mechanism is inert by construction.
